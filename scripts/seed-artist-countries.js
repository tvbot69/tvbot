const fs = require('fs');
const path = require('path');
const { PrismaClient } = require('@prisma/client');
require('dotenv/config');

const prisma = new PrismaClient();

const map = {
  // Egypt (EG)
  'abyusif': 'EG',
  'marwan pablo': 'EG',
  'marwan moussa': 'EG',
  'zaf': 'EG',
  'ziad zaza': 'EG',
  'tul8te': 'EG',
  'hady moamer': 'EG',
  'afroto': 'EG',
  'lella fadda': 'EG',
  'abo el anwar': 'EG',
  'mousv': 'EG',
  'young giza': 'EG',
  'lil baba': 'EG',
  'qetoo': 'EG',
  'ahmed santa': 'EG',
  'begad': 'EG',
  'wegz': 'EG',
  'hamo bika': 'EG',
  'sofyvnoxo': 'EG',
  'wezza montaser': 'EG',
  'wingii': 'EG',
  'muhab': 'EG',
  'lege-cy': 'EG',
  'mahdy madness': 'EG',
  'cairokee': 'EG',
  'shehab': 'EG',
  'hamo eltikha': 'EG',
  'arsenik': 'EG',
  'el waili': 'EG',
  'karim osama': 'EG',
  'dizzytooskinny': 'EG',
  'omar gangster': 'EG',
  'yonyo': 'EG',
  'hako': 'EG',
  'kurffew': 'EG',
  'fares sokar': 'EG',
  'fares sokar - فارس سكر': 'EG',
  'essam sasa': 'EG',
  'salah tayr': 'EG',
  'omar id': 'EG',
  'eldab3': 'EG',
  '3ab3az': 'EG',
  'moscow': 'EG',
  'husayn': 'EG',
  'karim enzo': 'EG',
  'ameen khattab': 'EG',
  'molotof': 'EG',
  'eslam kabonga': 'EG',
  'sharmoofers': 'EG',
  'amr diab': 'EG',
  'tamer hosny': 'EG',
  'mohamed mounir': 'EG',
  'angham': 'EG',
  'sherine': 'EG',
  'hamaki': 'EG',
  'bahaa sultan': 'EG',
  'oum kalthoum': 'EG',
  'abdel halim hafez': 'EG',
  'farid el atrache': 'EG',
  'asala': 'SY',
  'fairuz': 'LB',
  'nancy ajram': 'LB',
  'elissa': 'LB',
  'haifa wehbe': 'LB',
  'ragheb alama': 'LB',
  'myriam fares': 'LB',
  'al nather': 'PS',
  'shabjdeed': 'PS',
  'daboor': 'PS',
  'zaid khaled': 'JO',
  'akher zapheer': 'JO',
  'jadal': 'JO',
  'autostrad': 'JO',
  'aziz maraka': 'JO',

  // United States (US)
  'tv girl': 'US',
  'travis scott': 'US',
  'cigarettes after sex': 'US',
  'playboi carti': 'US',
  'the strokes': 'US',
  'deftones': 'US',
  'future': 'US',
  'd4vd': 'US',
  'the marías': 'US',
  'the marias': 'US',
  'odetari': 'US',
  'faye webster': 'US',
  '21 savage': 'US',
  'kanye west': 'US',
  'lumi athena': 'US',
  '$uicideboy$': 'US',
  'suicideboys': 'US',
  'the neighbourhood': 'US',
  'ken carson': 'US',
  'don toliver': 'US',
  'kendrick lamar': 'US',
  'yeat': 'US',
  'billie eilish': 'US',
  'elijah moon': 'US',
  'metro boomin': 'US',
  'lil tecca': 'US',
  'lil uzi vert': 'US',
  'rich amiri': 'US',
  'intomymind': 'US',
  'dominic fike': 'US',
  'lana del rey': 'US',
  'system of a down': 'US',
  'xxxtentacion': 'US',
  '6arelyhuman': 'US',
  'homixide gang': 'US',
  'lil baby': 'US',
  'korn': 'US',
  'frank ocean': 'US',
  'title fight': 'US',
  'trippie redd': 'US',
  'eyedress': 'US',
  'nirvana': 'US',
  'destroy lonely': 'US',
  'deko': 'US',
  'maebi': 'US',
  'tana': 'US',
  'baby keem': 'US',
  'pixies': 'US',
  'sewerperson': 'US',
  'yameii online': 'US',
  'elita': 'US',
  'jack harlow': 'US',
  'ice spice': 'US',
  'cannibal corpse': 'US',
  'vansire': 'US',
  'mitski': 'US',
  'frankie reyes': 'US',
  'take care': 'US',
  'mag.lo': 'US',
  'gunna': 'US',
  '¥$': 'US',
  'lonelyrari': 'US',
  'dzel uzi': 'US',
  'a$ap rocky': 'US',
  'asap rocky': 'US',
  'young thug': 'US',
  'tyler, the creator': 'US',
  'chezile': 'US',
  'jordana': 'US',
  'denzel curry': 'US',
  'taylor swift': 'US',
  'michael jackson': 'US',
  'eminem': 'US',
  'beyoncé': 'US',
  'beyonce': 'US',
  'rihanna': 'BB',
  'ariana grande': 'US',
  'lady gaga': 'US',
  'katy perry': 'US',
  'bruno mars': 'US',
  'linkin park': 'US',
  'metallica': 'US',
  'green day': 'US',
  'red hot chili peppers': 'US',
  'foo fighters': 'US',
  'slipknot': 'US',
  'blink-182': 'US',
  'paramore': 'US',
  'my chemical romance': 'US',
  'panic! at the disco': 'US',
  'fall out boy': 'US',
  'weezer': 'US',
  'the killers': 'US',
  'imagine dragons': 'US',
  'twenty one pilots': 'US',
  'post malone': 'US',
  'juice wrld': 'US',
  'lil peep': 'US',
  'sza': 'US',
  'doja cat': 'US',
  'olivia rodrigo': 'US',
  'sabrina carpenter': 'US',
  'chappell roan': 'US',
  'clairo': 'US',
  'phoebe bridgers': 'US',
  'boygenius': 'US',
  'sufjan stevens': 'US',
  'bon iver': 'US',
  'fleet foxes': 'US',
  'the national': 'US',
  'death cab for cutie': 'US',
  'modest mouse': 'US',
  'neutral milk hotel': 'US',
  'elliott smith': 'US',
  'jeff buckley': 'US',
  'bob dylan': 'US',
  'bruce springsteen': 'US',
  'elvis presley': 'US',
  'the beach boys': 'US',
  'talking heads': 'US',
  'the velvet underground': 'US',
  'sonic youth': 'US',
  'r.e.m.': 'US',
  'rem': 'US',
  'blondie': 'US',
  'the doors': 'US',
  'ramones': 'US',
  'the ramones': 'US',
  'pearl jam': 'US',
  'soundgarden': 'US',
  'alice in chains': 'US',
  'stone temple pilots': 'US',
  'rage against the machine': 'US',
  'audioslave': 'US',
  'tool': 'US',
  'avenged sevenfold': 'US',
  'limp bizkit': 'US',
  'disturbed': 'US',
  'three days grace': 'CA',
  'breaking benjamin': 'US',
  'skillet': 'US',
  'evanescence': 'US',
  'mac miller': 'US',
  'childish gambino': 'US',
  'j. cole': 'US',
  'j cole': 'US',
  'kali uchis': 'US',

  // Canada (CA)
  'drake': 'CA',
  'mac demarco': 'CA',
  'the weeknd': 'CA',
  'tory lanez': 'CA',
  'crystal castles': 'CA',
  'ghostly kisses': 'CA',
  'men i trust': 'CA',
  'justin bieber': 'CA',
  'shawn mendes': 'CA',
  'avril lavigne': 'CA',
  'celine dion': 'CA',
  'alanis morissette': 'CA',
  'rush': 'CA',
  'neil young': 'CA',
  'joni mitchell': 'CA',
  'leonard cohen': 'CA',
  'arcade fire': 'CA',
  'metric': 'CA',
  'simple plan': 'CA',
  'sum 41': 'CA',
  'carly rae jepsen': 'CA',
  'alessia cara': 'CA',
  'daniel caesar': 'CA',

  // United Kingdom (GB)
  'radiohead': 'GB',
  'arctic monkeys': 'GB',
  'black sabbath': 'GB',
  'central cee': 'GB',
  'the smiths': 'GB',
  'pinkpantheress': 'GB',
  'pink floyd': 'GB',
  'strawberry guy': 'GB',
  'nemzzz': 'GB',
  'sleep token': 'GB',
  'the beatles': 'GB',
  'queen': 'GB',
  'david bowie': 'GB',
  'led zeppelin': 'GB',
  'the rolling stones': 'GB',
  'the clash': 'GB',
  'the cure': 'GB',
  'joy division': 'GB',
  'new order': 'GB',
  'oasis': 'GB',
  'blur': 'GB',
  'pulp': 'GB',
  'the verve': 'GB',
  'coldplay': 'GB',
  'muse': 'GB',
  'gorillaz': 'GB',
  'the xx': 'GB',
  'florence + the machine': 'GB',
  'adele': 'GB',
  'ed sheeran': 'GB',
  'dua lipa': 'GB',
  'harry styles': 'GB',
  'sam smith': 'GB',
  'charli xcx': 'GB',
  'beabadoobee': 'GB',
  'bring me the horizon': 'GB',
  'iron maiden': 'GB',
  'judas priest': 'GB',
  'deep purple': 'GB',
  'the who': 'GB',
  'the police': 'GB',
  'depeche mode': 'GB',
  'tears for fears': 'GB',
  'pet shop boys': 'GB',
  'portishead': 'GB',
  'massive attack': 'GB',
  'aphex twin': 'GB',
  'burial': 'GB',

  // Australia (AU)
  'tame impala': 'AU',
  'vacations': 'AU',
  'ac/dc': 'AU',
  'acdc': 'AU',
  'inxs': 'AU',
  'silverchair': 'AU',
  'king gizzard & the lizard wizard': 'AU',
  'sia': 'AU',
  'troye sivan': 'AU',
  'the kid laroi': 'AU',
  '5 seconds of summer': 'AU',
  'gotye': 'AU',
  'vance joy': 'AU',
  'flume': 'AU',
  'empire of the sun': 'AU',
  'nick cave': 'AU',
  'nick cave & the bad seeds': 'AU',

  // Japan (JP)
  'joji': 'JP',
  'babymetal': 'JP',
  'yoasobi': 'JP',
  'ado': 'JP',
  'radwimps': 'JP',
  'one ok rock': 'JP',
  'kenshi yonezu': 'JP',
  'fujii kaze': 'JP',
  'hikaru utada': 'JP',
  'joe hisaishi': 'JP',
  'ryuichi sakamoto': 'JP',
  'nujabes': 'JP',
  'fishmans': 'JP',
  'casiopea': 'JP',
  'masayoshi takanaka': 'JP',
  'lamp': 'JP',
  'toe': 'JP',
  'boris': 'JP',

  // South Korea (KR)
  'bts': 'KR',
  'blackpink': 'KR',
  'stray kids': 'KR',
  'twice': 'KR',
  'newjeans': 'KR',
  'aespa': 'KR',
  'exo': 'KR',
  'seventeen': 'KR',
  'red velvet': 'KR',
  'txt': 'KR',
  'enhypen': 'KR',
  'itzy': 'KR',
  'le sserafim': 'KR',
  'ive': 'KR',
  'ateez': 'KR',
  'iu': 'KR',
  'psy': 'KR',

  // France (FR)
  'daft punk': 'FR',
  'phoenix': 'FR',
  'justice': 'FR',
  'air': 'FR',
  'm83': 'FR',
  'kavinsky': 'FR',
  'aya nakamura': 'FR',
  'pnl': 'FR',
  'gazo': 'FR',
  'tiakola': 'FR',
  'booba': 'FR',
  'nekfeu': 'FR',
  'orelsan': 'FR',

  // Germany (DE)
  'rammstein': 'DE',
  'kraftwerk': 'DE',
  'scorpions': 'DE',
  'tokio hotel': 'DE',
  'milky chance': 'DE',
  'zedd': 'DE',

  // Sweden (SE)
  'abba': 'SE',
  'avicii': 'SE',
  'swedish house mafia': 'SE',
  'roxette': 'SE',
  'europe': 'SE',
  'ghost': 'SE',
  'meshuggah': 'SE',
  'opeth': 'SE',
  'in flames': 'SE',
  'bladee': 'SE',
  'yung lean': 'SE',
  'ecco2k': 'SE',
  'zara larsson': 'SE',
  'tove lo': 'SE',

  // Norway (NO)
  'burzum': 'NO',
  'girl in red': 'NO',
  'a-ha': 'NO',
  'kygo': 'NO',
  'alan walker': 'NO',
  'aurora': 'NO',
  'darkthrone': 'NO',
  'mayhem': 'NO',

  // Ireland (IE)
  'sign crushes motorist': 'IE',
  'u2': 'IE',
  'the cranberries': 'IE',
  'hozier': 'IE',
  'fontaines d.c.': 'IE',
  'my bloody valentine': 'IE',
  'inhaler': 'IE',

  // Belgium (BE)
  'dina ayada': 'BE',
  'stromae': 'BE',
  'damso': 'BE',
  'angèle': 'BE',
  'angele': 'BE',

  // Georgia (GE)
  'psychonaut 4': 'GE',

  // Switzerland (CH)
  'zeal & ardor': 'CH',

  // Netherlands (NL)
  'martin garrix': 'NL',
  'tiësto': 'NL',
  'tiesto': 'NL',
  'armin van buuren': 'NL',
  'hardwell': 'NL',
  'afrojack': 'NL',

  // Puerto Rico (PR)
  'bad bunny': 'PR',
  'daddy yankee': 'PR',
  'don omar': 'PR',
  'ozuna': 'PR',
  'anuel aa': 'PR',
  'rauw alejandro': 'PR',
  'jhayco': 'PR',
  'myke towers': 'PR',

  // Colombia (CO)
  'shakira': 'CO',
  'j balvin': 'CO',
  'maluma': 'CO',
  'karol g': 'CO',
  'morat': 'CO',

  // Mexico (MX)
  'peso pluma': 'MX',
  'natanael cano': 'MX',
  'junior h': 'MX',
  'christian nodal': 'MX',
  'luis miguel': 'MX',
  'caifanes': 'MX',
  'molotov': 'MX',

  // Brazil (BR)
  'sepultura': 'BR',
  'anitta': 'BR',
  'alok': 'BR',
  'caetano veloso': 'BR',
  'antonio carlos jobim': 'BR',
  'jorge ben jor': 'BR',

  // Spain (ES)
  'rosalía': 'ES',
  'rosalia': 'ES',
  'enrique iglesias': 'ES',
  'c. tangana': 'ES',
  'quevedo': 'ES',
  'morad': 'ES',
  'bad gyal': 'ES',
  'rels b': 'ES',
};

const jsonPath = path.join(process.cwd(), 'src', 'bot', 'resources', 'artist_countries.json');
fs.writeFileSync(jsonPath, JSON.stringify(map, null, 2), 'utf8');
console.log('Saved', Object.keys(map).length, 'artists to', jsonPath);

// Sync to PostgreSQL DB
(async () => {
  let updated = 0;
  for (const [name, code] of Object.entries(map)) {
    try {
      const res = await prisma.artist.updateMany({
        where: {
          name: { equals: name, mode: 'insensitive' },
          countryCode: null,
        },
        data: {
          countryCode: code,
        },
      });
      updated += res.count;
    } catch {}
  }
  console.log('Updated', updated, 'artists in Postgres database with country codes!');
  await prisma.$disconnect();
})();
