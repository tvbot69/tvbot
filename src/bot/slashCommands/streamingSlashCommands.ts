...
export class StreamingSlashCommands implements ISlashCommandModule {
  ...
  private async resolveQuery(ctx: ContextModel): Promise<string> {
  ...
  public async spotifyTrackSlashAsync(ctx: ContextModel): Promise<ResponseModel> {
    ...
    const query = await this.resolveQuery(ctx);
    ...
    if (!query) {
      ...
      container.addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          `### 🟢 Spotify Search\nPlease specify a song to search for:\n> \`/spotify query:<song name>\``,
        ),
      );
