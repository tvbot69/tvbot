...
export class SettingsInteractions {
  ...
  public async handleSettingsButton(interaction: ButtonInteraction): Promise<void> {
    ...
    if (action === 'fmmode') {
      await interaction.reply({
        content: 'Use `/fmmode` to customize your .fm — embed type, footer, buttons, and small text.',
        flags: MessageFlags.Ephemeral,
      });
      return;
    }
    ...
    if (action === 'serverfm') {
      await interaction.reply({
        content: 'Use `/settings` Server tab → set guild fmEmbedType via `/servermode`. Channel overrides per-channel.',
        flags: MessageFlags.Ephemeral,
      });
      return;
    }
