  ...
  public async getOrCreateTracksBulk(
    ...
    for (let i = 0; i < list.length; i += chunkSize) {
      ...
      if (missing.length > 0) {
        ...
        const twins = await this.prisma.$queryRaw<Array<{ trackId: number; name: string; artistId: number }>>`
      ...
      if (stillMissing.length > 0) {
        await this.prisma.track.createMany({ data: stillMissing, skipDuplicates: true });
